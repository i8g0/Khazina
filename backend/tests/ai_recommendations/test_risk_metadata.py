...
def test_build_risk_metadata_supplement_includes_findings() -> None:
    ...
    assert "سياق التحليل الحتمي" in supplement
    assert "Risk Engine" in supplement
    assert metadata["risk_analysis"]["overall_posture_level"] in supplement
    assert metadata["risk_findings"][0]["finding_id"] in supplement
